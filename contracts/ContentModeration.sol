...
contract ContentModeration {
	...
	mapping(address => uint) usersToActiveDisputeCount;
	RNG rngContract;
	Post postContract;
	Token tokenContract;
	...
	constructor(RNG _rngContract, Post _postContract, Token _tokenContract) {
	...
	function openDispute(uint postId, string memory reason) public {
		...
		usersToActiveDisputeCount[msg.sender]++;
	}
	...
	function getNewRandomDispute() public returns (uint) {
		uint n = activeDisputes.length;
		// prevent disputers from voting on their own dispute
		require(usersToActiveDisputeCount[msg.sender] == 0, "you have an open dispute, therefore you cannot vote until your dispute is closed");
	...
	function getCurrentDispute() public view returns (uint) {
	...
	function getCurrentDisputeOfUser(address user) public view returns (uint) {
		require(msg.sender == address(postContract), "only post contract can call getCurrentDisputeOfUser");
	...
	function endDispute(uint disputeId) public {
		...
		if (numVoters <= MIN_VOTE_COUNT || numApprovers == numRejectors) {
			...
			// 2. the total reward pool (dispute.voterRewardPool + 1/n of contract's rewardPoool) is distributed amongst the approvers
			// n = no. of activeDisputes
			uint totalRewardPool = dispute.voterRewardPool + rewardPool / n;
			uint rewardPerVoter = totalRewardPool / numApprovers;
			rewardPool -= rewardPool / n;
			...
			// 1. creator's locked tokens are not returned and are added to the contract's rewardPool.
			// 2. the total reward pool (dispute.voterRewardPool + 1/n of contract's rewardPool) is distributed amongst the rejectors
			// n = no. of activeDisputes
			rewardPool += dispute.lockedTokens; // add creator's locked tokens to contract's reward pool
			uint totalRewardPool = dispute.voterRewardPool + rewardPool / n;
		...
		// --- clean up ---
		// decrease number of active dispute for the dispute owner
		usersToActiveDisputeCount[dispute.owner]--; 
		// remove the dispute id from the list of active disptues
